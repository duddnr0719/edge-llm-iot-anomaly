...
V10_TRANSFER="$DIST/v10_transfer"
HF_PATH="$DIST/sensor-merged-hf-v10"
MLC_OUT="Qwen2.5-3B-sensor-v10-q4f16_1"

echo "=== [1/5] 디스크 정리 ==="
rm -rf "$V10_TRANSFER" 2>/dev/null && echo "v10_transfer 초기화"
df -h "$DIST"

echo "=== [2/5] V10 모델 파일 다운로드 ==="
mkdir -p "$V10_TRANSFER"
cd "$V10_TRANSFER"
...
docker run --rm --runtime=nvidia --network=host \
    ...
    mlc_llm convert_weight /dist/sensor-merged-hf-v10 \
    --quantization q4f16_1 -o /dist/$MLC_OUT

docker run --rm --runtime=nvidia --network=host \
    -v "$DIST:/dist" dustynv/mlc:0.20.0-r36.4.0 \
    mlc_llm gen_config /dist/sensor-merged-hf-v10 \
    --quantization q4f16_1 -o /dist/$MLC_OUT --conv-template qwen2

echo "=== 서비스 업데이트 ==="
sudo sed -i 's/sensor-v8-q4f16_1/sensor-v10-q4f16_1/g' /etc/systemd/system/mlc-v5.service
sudo sed -i 's/MLC LLM V8/MLC LLM V10/g' /etc/systemd/system/mlc-v5.service
sudo systemctl daemon-reload && sudo systemctl restart mlc-v5
echo "서비스 재시작 완료"

echo "=== 정리 ==="
rm -rf "$V10_TRANSFER" "$HF_PATH"
df -h "$DIST"
echo "=== V10 배포 완료 ==="
