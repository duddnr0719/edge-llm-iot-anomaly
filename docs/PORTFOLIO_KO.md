...
- [학습 히스토리 V7→V14](TRAINING_HISTORY.md) — 단계별 진화, V13 하이퍼파라미터·실패 12건 분석, V14 붕괴 상세, 정확도 천장 6요인
- [33케이스 벤치마크 명세](BENCHMARK.md) — 전체 케이스 표(입력값·기대 판정), 경계값 케이스, 채점 방식
- [19일 실운영 로그 분석](DEPLOYMENT_LOG.md) — 지연 백분위, 판정 분포, 교정 필드 분석, 벤치마크 실패 패턴의 실환경 재현
